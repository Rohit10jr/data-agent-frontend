...
import { CloudUpload, FileCheck, X, Database } from 'lucide-react';
...
const SOURCE_OPTIONS: { value: SourceType; label: string; hint: string }[] = [
	{ value: 'database', label: 'Database connection string', hint: 'PostgreSQL, MySQL, MS SQL, Snowflake' },
	{ value: 'sqlite', label: 'SQLite file', hint: '.sqlite / .db' },
	{ value: 'csv', label: 'CSV file', hint: '.csv — converted to a SQLite table' },
	{ value: 'excel', label: 'Excel file', hint: '.xlsx — each sheet becomes a table' },
	{ value: 'sas7bdat', label: 'SAS file', hint: '.sas7bdat — converted to SQLite' },
];
...
export function ConnectionFormDialog({ open, onOpenChange, connection }: ConnectionFormDialogProps) {
	...
	const createDsnMut = useMutation({
		...
		onError: (err) => {
			setError(err instanceof ApiError ? err.message : (err as Error)?.message ?? 'Failed to create connection');
		},
	});
	...
	const submitting = createDsnMut.isPending || createFileMut.isPending || updateMut.isPending;
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className='sm:max-w-lg max-h-[85vh] overflow-y-auto'>
				...
				<form onSubmit={handleSubmit} className='space-y-4'>
					...
					{/* When editing, only the DSN and name are editable. File replacement is not supported. */}
					{isEdit ? (
						<div className='space-y-1.5'>
						...
						<>
							...
							<fieldset className='space-y-2'>
								<legend className='text-sm font-medium mb-1'>Source type</legend>
								<div className='space-y-1.5'>
